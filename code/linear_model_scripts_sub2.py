...
fig = plt.figure()
i = 1
run1_house = raw_beta_vols["%s_run001" % subid][:, 25:50, 32:37, match_para["house"]]
for z in range(5):
    ...
    if z == 2:
        plt.title("%s_run1_house" % subid)
run2_house = raw_beta_vols["%s_run002" % subid][:, 25:50, 32:37, match_para["house"]]
for z in range(5):
    plt.subplot(3, 5, i, xticks=[], yticks=[])
    ...
    if z == 2:
        plt.title("%s_run2_house" % subid)
run2_face = raw_beta_vols["%s_run002" % subid][:, 25:50, 32:37, match_para["face"]]
for z in range(5):
    plt.subplot(3, 5, i, xticks=[], yticks=[])
...
house1_3d = np.ravel(raw_beta_vols_3d_corr["%s_run001" % subid][:, 25:50, 32:37, match_para["house"]])
house2_3d = np.ravel(raw_beta_vols_3d_corr["%s_run002" % subid][:, 25:50, 32:37, match_para["house"]])
face2_3d = np.ravel(raw_beta_vols_3d_corr["%s_run002" % subid][:, 25:50, 32:37, match_para["face"]])
...
# add up even run results / odd run results and take mean for each groups
for item in object_list:
    even_run_3d[item] = np.zeros_like(raw_beta_vols_3d_corr["%s_run001" % subid][:, 25:50, 32:37, match_para[item]])
    odd_run_3d[item] = np.zeros_like(raw_beta_vols_3d_corr["%s_run001" % subid][:, 25:50, 32:37, match_para[item]])
    print ("make average of \"3D\" odd run results:")
    # add up odd runs results
    for i in range(1, run_num+1, 2):
        temp = raw_beta_vols_3d_corr["%s_run0%02d" % (subid, i)][:, 25:50, 32:37, match_para[item]]
        temp[np.isnan(temp)] = 0
