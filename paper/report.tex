...
The dataset for the study could be found on OpenfMRI database\cite{fmriData} 
with assession number of ds105, curated and available for download. The
sub-directory of ds105 contains files that describe the details of this study: 
general information of this study (README file), research articles 
associated with this dataset (references.txt), 
...
Individual subject has their own directory to store the detail results. In each 
of these directories, there are four sub-directories: anatomy, behav, BOLD 
and model. The anatomy sub-directory contains the high resolution scanning
for the head of the subject (highres001.nii.gz), the mask to get the 
``brain only'' portion from the scan 
(highres001\textunderscore brain\textunderscore mask.nii.gz) 
...
The ``model" sub-directory contains detail 
...
The ``BOLD" sub-directory contains the fMRI results for all the 12 
